...
export class RoomsService {
  ...
  async createRoom(participants: string[]) {
    const res = await this.roomsRepository.find({
  ...
  async getAllRoomsIds(idUser: string) {
    ...
    const roomsIds = [];
    await Promise.all(
      user.friends.map(async (idFriend) => {
  ...
  async getRoomInfo(roomId: string) {
    ...
    const participants = await Promise.all(
      room.participants.split(',').map(async (id) => {
  ...
  async getLastMessages(userId: string) {
    ...
    const lastMessages = {};
    await Promise.all(
      user.groupRooms.map(async (id) => {
        const messages = await this.messagesService.getAllRoomMessages(id);
        if (messages.length > 1) {
          lastMessages[id] = messages.sort(
            (a, b) => Number(a.sendingDate) - Number(b.sendingDate),
          )[messages.length - 1];
    ...
    const friendRoomsParticipants = user.friends.map((id) =>
      [id, userId].sort().toString(),
    );
    await Promise.all(
      friendRoomsParticipants.map(async (participants) => {
