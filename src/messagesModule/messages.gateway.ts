  ...
  implements OnGatewayInit, OnGatewayConnection, OnGatewayDisconnect
{
  ...
  async connection(client: Socket) {
    ...
    const friendsOnline = user.friends.filter((friendId) =>
      this.online.some((id) => id === friendId),
    );
  ...
  async handleMessage(client: Socket, payload: messagePayloadDto) {
    ...
    this.server
      .to(payload.roomId)
      .emit(`messages:add`, [{ ...result, senderAvatar: sender.imagePath }]);
  }
  ...
  async getMessage(client: Socket, payload) {
    const clientId = client.handshake.query.id;
    const messages = await this.messagesService.getAllRoomMessages(
      payload.roomId,
    );
    this.server
      .to(payload.roomId)
      .emit(`messages:get${clientId}`, [...messages]);
  }
  ...
  async getLastMessages(client: Socket) {
    const clientId = client.handshake.query.id;
    const messages = await this.messagesService.getLastMessages(
      clientId.toString(),
    );
    this.server
      .to(clientId)
      .emit(`messages:getLastMessages${clientId}`, messages);
  }
  ...
  async handleDisconnect(client: Socket) {
    ...
    user.friends.map((id) =>
      this.server.to(id).emit('friends:wentOffline', clientId),
    );
  }
  ...
  async handleConnection(client: Socket, ...args: any[]) {
    ...
    user.friends.map((id) =>
      this.server.to(id).emit('friends:wentOnline', clientId),
    );
  }
}
